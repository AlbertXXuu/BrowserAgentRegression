...
DEFAULT_PORT = 7870
ASSET_SHA256 = {
    "assets/brand/alvenx-wordmark.svg": "6cc422fb2ed289bee723f1c6e6d19baec63c18d988616eb5b90d8332a30b7b1e",
...
<body>
  <div class="page-frame">
    <header class="site-header liquid-surface">
      <button type="button" class="brand-link" data-alvenx-home aria-label="AlvenX — Back to top">
        <img src="/assets/wordmark.svg" alt="AlvenX">
      </button>
      <nav aria-label="Page sections">
        <a href="#evidence">Evidence</a><a href="#tasks">Tasks</a><a href="#boundary">Method</a>
      </nav>
      <span class="local-badge"><i aria-hidden="true"></i>Studio v1.1.2 · Evidence v1.0.0</span>
    </header>
        ...
        <div class="hero-copy">
          ...
          <div class="hero-actions">
            <button class="liquid-button" id="run-demo" type="button">
              <span class="link-label">Run local demo</span><span class="link-arrow" aria-hidden="true">↗</span>
            </button>
            ...
            <a class="text-link" href="#evidence"><span class="link-label">Inspect preserved v1 evidence</span><span class="link-arrow" aria-hidden="true">↓</span></a>
          </div>
...
body{min-width:0}.brand-link{min-height:44px;align-items:center}.site-header nav a{display:inline-flex;min-height:44px;align-items:center}.liquid-button,.text-link{text-decoration:none}.text-link{display:inline-flex;min-height:44px;align-items:center;gap:.35em}.text-link .link-label{text-decoration:underline;text-underline-offset:.14em}.link-arrow{display:inline-block;flex:0 0 auto;text-decoration:none}.liquid-button .link-arrow{font-weight:700}.hero-proof h2{margin:22px 0 0;color:var(--primary);font-size:clamp(1.35rem,2vw,1.85rem);font-weight:570;letter-spacing:-.025em;line-height:1.08}.checkpoint-plot{margin:22px 0 18px;padding:18px;border:1px solid rgb(71 105 148 / 16%);border-radius:20px;background:rgb(251 253 255 / 66%)}.plot-loading{color:var(--muted);font-size:.82rem}.checkpoint-grid{display:grid;grid-template-columns:82px repeat(3,minmax(0,1fr));gap:12px 8px;align-items:center}.series-label{color:var(--reading);font-size:.75rem;font-weight:650}.checkpoint-node{display:grid;min-width:0;min-height:82px;align-self:stretch;align-content:start;grid-template-columns:minmax(0,1fr);grid-template-rows:12px minmax(20px,auto) minmax(20px,auto);place-items:center;gap:4px;padding:9px 4px 5px;border:1px solid rgb(71 105 148 / 14%);border-radius:14px;background:rgb(255 255 255 / 58%)}.checkpoint-node i{width:12px;height:12px;border-radius:50%}.checkpoint-node b{font-size:.78rem}.checkpoint-node.reference i{background:var(--blue);box-shadow:0 0 18px rgb(37 99 235 / 38%)}.checkpoint-node.candidate i{background:var(--violet)}.checkpoint-node.first-failure{border-color:rgb(124 58 237 / 42%);background:rgb(237 233 254 / 62%);box-shadow:0 0 0 3px rgb(124 58 237 / 8%)}.checkpoint-node small{width:100%;min-height:20px;color:#6d28d9;font-size:.75rem;font-weight:680;line-height:1.25;overflow-wrap:normal;word-break:normal;text-align:center}.checkpoint-label{min-width:0;color:var(--muted);font-size:.8125rem;line-height:1.4;overflow-wrap:normal;word-break:normal;text-align:center}.hero-proof #proof-caption{margin:0;color:var(--reading);font-size:.88rem;line-height:1.62}
...
@media(max-width:599px){.hero-proof{padding:22px}.checkpoint-plot{padding:12px}.checkpoint-grid{grid-template-columns:repeat(3,minmax(0,1fr))}.series-label{grid-column:1/-1}.checkpoint-spacer{display:none}}
...
STUDIO_JS = r"""const $=s=>document.querySelector(s),$$=s=>document.querySelectorAll(s);const pct=v=>`${Math.round(v*100)}%`,checkpointLabel=id=>id.replace(/^[^.]+\./,"").replaceAll("."," ").replaceAll("_"," ").replace(/^./,c=>c.toUpperCase());
function renderCheckpoint(task){const plot=$("#checkpoint-plot"),nodes=(key,kind)=>task.checkpoints.map(c=>`<span class="checkpoint-node ${kind} ${kind==="candidate"&&c.id===task.firstFailure?"first-failure":""}"><i></i><b>${pct(c[key])}</b><small>${kind==="candidate"&&c.id===task.firstFailure?"First divergence":""}</small></span>`).join(""),labels=task.checkpoints.map(c=>`<span class="checkpoint-label" title="${c.id}">${checkpointLabel(c.id)}</span>`).join("");plot.innerHTML=`<div class="checkpoint-grid"><strong class="series-label">Reference</strong>${nodes("referenceRate","reference")}<strong class="series-label">Popup-blind</strong>${nodes("candidateRate","candidate")}<span class="checkpoint-spacer"></span>${labels}</div>`;plot.setAttribute("aria-label",`${task.label}: reference and popup-blind checkpoint pass rates; first divergence ${task.firstFailure}`);$("#proof-caption").textContent=`${task.label} · popup overlay. Reference passes all ${task.checkpoints.length} checkpoints; popup-blind first fails at ${task.firstFailure} (${pct(task.agreement)} repetition agreement).`;}
...
function backToTop(){
  ...
  window.scrollTo({top:0,left:window.scrollX,behavior:window.matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth'});
}
...
window.addEventListener("DOMContentLoaded",()=>{$("[data-alvenx-home]").addEventListener("click",backToTop);load().catch(error=>{$("#run-status").textContent=error.message;});$("#run-demo").addEventListener("click",run);syncNavigation();});
"""
